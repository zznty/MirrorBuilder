...
@{
    ...
    versions      = $forgeVersions.versions.Values | ForEach-Object {
        [PSCustomObject]@{
            releaseTime = EmptyToNull (if (Test-Path "$PSScriptRoot/meta-upstream/forge/version_manifests/$($_.longversion).json") { (Get-Content "$PSScriptRoot/meta-upstream/forge/version_manifests/$($_.longversion).json" | ConvertFrom-Json | Select-Object -ExpandProperty releaseTime) } else { "1970-01-01T00:00:00+00:00" });
            version     = $_.version
